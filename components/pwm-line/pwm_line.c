...
/// @return 
static int pwmDutyTicksCalculate(int duty,uint8_t timer_size,uint8_t duty_cut){
    ...
    return (duty_ticks-duty_cut_ticks);
...
/// @param timer_size timer bits
/// @return 
static int pwmLagTicksCalculate(uint8_t phase,uint8_t timer_size){

    int total_ticks=(1<<timer_size); //2^13
    int phase_percentage=((int)phase*100)/360;               //Phase_in_Degrees/Total_Degress * 100
    int phase_ticks=(phase_percentage*total_ticks)/100;
...
int pwmCreate(pwm_line_t* self,pwm_config_t*  config){
    ...
    int duty_cycle=pwmDutyTicksCalculate(config->duty_cycle,LEDC_DUTY_RES,config->phase_cut_on_duty);
    int lag=pwmLagTicksCalculate(config->phase,LEDC_DUTY_RES);
